...
xs = data_sampler.sample_xs(b_size=batch_size, n_points=4) 
for i in range(xs.shape[0]):      
    for j in range(xs.shape[1]):    
        for k in range(xs.shape[2]):  
            torch.manual_seed(100*i + 10*j +k)
                #xs[i, j, k] = torch.randn(1)  # Random value in U(-1,1)
            #xs[i, j, k] = 950 + 100*torch.rand(1)
            xs[i, j, k] = -1*j #torch.rand(1)  # Random value in N(0,1)
